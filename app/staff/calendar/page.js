...
export default function StaffCalendarPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-2 py-4 md:px-4 md:py-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 rounded-md border border-white/15 bg-white/5 p-1">
            ...
            <button onClick={today} className="display px-3 py-1 text-xs text-white/70 hover:text-mw-pink">Aujourd'hui</button>
      ...
      {view === 'week' && <WeekView date={date} activities={bookable} bookings={bookings} blocks={blocks} onSelect={setSelected} />}
      {view === 'month' && <MonthView date={date} activities={bookable} bookings={bookings} onChangeDate={setDateStr} />}
...
// ============================================================
// DAY VIEW — lanes verticales par activité + timeline
// ============================================================
function DayView({ date, activities: acts, bookings, blocks, onSelect, hours }) {
  if (!hours) {
    return (
  ...
  return (
    <div className="overflow-x-auto rounded-2xl border border-white/10 bg-white/[0.02]">
      <div className="flex min-w-max">
        {/* Time column */}
        <div className="sticky left-0 z-10 w-12 shrink-0 border-r border-white/10 bg-mw-darker">
          <div className="h-12 border-b border-white/10" />
          {Array.from({ length: hourCount }).map((_, i) => (
            <div key={i} className="display flex h-16 items-start justify-end border-b border-white/5 pr-1 pt-1 text-[10px] text-white/40">
              {fromMinutes(openM + i * 60)}
            </div>
          ))}
        </div>
        ...
        {acts.map((a) => {
          ...
          return (
            <div key={a.id} className="w-40 shrink-0 border-r border-white/10">
              <div className="sticky top-0 z-10 flex h-12 items-center gap-2 border-b border-white/10 bg-mw-darker px-2">
              ...
              <div className="relative" style={{ height: `${hourCount * 64}px` }}>
                {/* hour grid */}
                {Array.from({ length: hourCount }).map((_, i) => (
                  <div key={i} className="absolute left-0 right-0 border-b border-white/5" style={{ top: `${i * 64}px`, height: '64px' }} />
                ))}

                {/* slots */}
                {slots.map((slot) => {
                  const slotMinutes = toMinutes(slot.start) - openM;
                  const top = (slotMinutes / 60) * 64;
                  const height = (a.duration / 60) * 64 - 1;
                  const slotItems = activityBookings.filter((i) => i.start === slot.start);
                  const players = slotItems.reduce((s, i) => s + i.players, 0);
                  const groupsCount = slotItems.length;
                  ...
                  if (block) bg = 'bg-mw-red/30 border-mw-red hover:bg-mw-red/40';
                  ...
                  return (
                    <button
                      key={slot.start}
                      onClick={() => onSelect({ ...slot, activityId: a.id, activity: a, date, items: slotItems, block })}
                    ...
                    >
                      ...
                      {!block && slotItems.length > 0 && (
                        <div className="w-full truncate text-[9px] text-white/50">
                          {groupsCount} groupe{groupsCount > 1 ? 's' : ''}
                        </div>
                      )}
                    </button>
                  );
...
// ============================================================
// WEEK VIEW — lignes par activité, colonnes par jour
// ============================================================
function WeekView({ date, activities: acts, bookings, blocks, onSelect }) {
      ...
      <table className="w-full min-w-max text-xs">
        <thead>
          <tr className="border-b border-white/10">
            <th className="display sticky left-0 z-10 w-40 bg-mw-darker px-2 py-2 text-left">Activité</th>
            {days.map((d) => {
        ...
        <tbody>
          {acts.map((a) => (
            <tr key={a.id} className="border-b border-white/5">
              <td className="sticky left-0 z-10 bg-mw-darker px-2 py-3">
              ...
              {days.map((d) => {
                ...
                const players = items.reduce((s, i) => s + i.players, 0);
...
// ============================================================
// MONTH VIEW — heatmap par jour
// ============================================================
function MonthView({ date, activities: acts, bookings, onChangeDate }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02]">
      <div className="grid grid-cols-7 border-b border-white/10 bg-mw-darker">
      ...
      <div className="grid grid-cols-7">
        {cells.map((cell, idx) => {
          ...
          const totalPlayers = dayBookings.reduce((s, b) => s + b.players, 0);
          const totalRevenue = dayBookings.reduce((s, b) => s + b.total, 0);
          const isToday = toDateStr(new Date()) === cell;
          const rate = totalPlayers / 40; // rough scale
          let color = '';
...
// SLOT DETAIL PANEL
// ============================================================
function SlotDetailPanel({ slot, onClose, onBlock, onUnblock, bookings }) {
  const [reason, setReason] = useState('');
  const [note, setNote] = useState('');

  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 backdrop-blur-sm md:items-center">
      <div className="w-full max-w-lg rounded-t-2xl border-t border-mw-pink/50 bg-mw-darker p-5 md:rounded-2xl md:border">
        <div className="mb-4 flex items-start justify-between">
          <div>
            <div className="display text-2xl">{slot.activity.name}</div>
            <div className="text-sm text-white/60">
        ...
        {slot.block ? (
          <div className="rounded-xl border border-mw-red/40 bg-mw-red/10 p-4">
            <div className="display mb-1 text-mw-red">🔒 Créneau bloqué</div>
            <div className="text-xs text-white/70">
              Raison : {slot.block.block_reason || slot.block.reason || '—'}
            </div>
            {(slot.block.note || slot.block.notes) && (
              <div className="mt-1 text-xs text-white/50">{slot.block.note || slot.block.notes}</div>
            )}
            ...
            {slot.items && slot.items.length > 0 ? (
              <div className="mb-4 space-y-2">
                <div className="display text-xs text-mw-pink">
                  {slot.items.length} groupe{slot.items.length > 1 ? 's' : ''} · {slot.items.reduce((s, i) => s + i.players, 0)}/{slot.activity.maxPlayers} joueurs
                </div>
                {slot.items.map((i, idx) => (
                  <div key={idx} className="rounded-xl bg-white/5 p-3 text-xs">
              ...
              <div className="mb-4 rounded-xl bg-white/5 p-4 text-center text-sm text-white/50">
                Créneau libre
              </div>
            )}

            <div className="rounded-xl border border-white/10 p-3">
              <div className="display mb-2 text-xs text-white/70">Bloquer ce créneau</div>
              ...
              />
              <button onClick={() => onBlock(reason, note)} disabled={!reason} className="btn-primary w-full !py-2 text-xs">
                Bloquer
              </button>
            </div>
          </>
