...
import { signRef } from '@/lib/signedToken';
...
export function buildConfirmationEmail({ booking, config, baseUrl }) {
  ...
  // Tokens HMAC pour sécuriser les liens (anti-énumération de refs).
  let token = '';
  try { token = signRef(ref); } catch { /* secret manquant en dev → token vide → endpoints renverront 401 */ }
  const invoiceUrl = `${baseUrl}/api/invoice?ref=${encodeURIComponent(ref)}&token=${token}`;
  const icsUrl = `${baseUrl}/api/booking-ics?ref=${encodeURIComponent(ref)}&token=${token}`;
