      ...
      properties: {
        ...
        items: {
          ...
          items: {
            ...
            required: ['activityId', 'date', 'start', 'end', 'players', 'unit', 'total'],
          },
        },
  ...
  switch (name) {
    ...
    case 'checkAvailability': {
      ...
      if (!isMultiRoom) {
        ...
        return {
          ...
          availableSlots: freeSlots.slice(0, 60),
        };
      }
      ...
      return {
        ...
        pricePerPlayer: price,
        totalForGroup: price * players,
        availableSlots: slotAnalysis.slice(0, 60),
    ...
    case 'createDraftBooking': {
      ...
      if (!EMAIL_RE.test(email)) {
        return { error: `Email "${customer?.email}" invalide. Redemande au client une adresse email correcte (format nom@domaine.ext) avant de recréer la résa.` };
      }
      if (!Array.isArray(items) || items.length === 0) {
        return { error: 'Aucun item dans la résa. Vérifie les activités/créneaux avant de retenter.' };
      }
      ...
      const booking = {
        ...
        items: items.map((i) => {
          ...
          const end = i.end || (act ? fromMinutes(toMinutes(i.start) + act.duration) : i.start);
          return {
      ...
      try {
        ...
        console.error('[createDraftBooking] FAILED', {
          bookingId: booking.id,
          email,
          items: booking.items.map((i) => ({ activityId: i.activityId, start: i.start, end: i.end, players: i.players, roomId: i.roomId })),
          error: e?.message,
          stack: e?.stack,
          pgDetails: e?.details || e?.hint || null,
        });
        return { error: `Création résa échouée: ${String(e.message || e)}. Escalate si 2ème échec.` };
      }
    }
