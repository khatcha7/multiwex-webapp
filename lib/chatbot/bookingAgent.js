  ...
  switch (name) {
    ...
    case 'checkAvailability': {
      ...
      if (!isMultiRoom) {
        ...
        const occupancy = await getSlotOccupancy(activityId, date, targetRoomId);
        const baseCap = targetRoomId ? rooms[0].maxPlayers : act.maxPlayers;

        const freeSlots = allSlots.map((slot) => {
          const occInfo = (occupancy[targetRoomId || '_'] || {})[slot.start];
      ...
      // Pour chaque slot, on collecte la dispo PAR ROOM.
      const occByRoom = {};
      ...
      const slotAnalysis = allSlots.map((slot) => {
        const roomAvailability = rooms.map((r) => {
          const occInfo = (occByRoom[r.id][r.id] || {})[slot.start];
          const playersInSlot = occInfo?.players || 0;
...
4. Tu es CONCISE. Phrases courtes. 2-3 phrases max par message. Jamais de "je serai ravie de".
5. Tu tutoies par défaut. Vouvoies si le client vouvoie.
6. Tu détectes la langue (FR, NL, EN) et réponds dans celle-ci.
7. Flow : comprendre besoin → checkAvailability → confirmer détails → récupérer prénom+nom+email → createDraftBooking → generatePaymentLink → donner lien paiement.
8. Tu ne crées une résa QUE si le client a confirmé l'activité + date + créneau + nombre joueurs + a donné prénom + nom + email valides.
9. Après createDraftBooking, tu appelles IMMÉDIATEMENT generatePaymentLink et tu donnes le lien.
10. Si cas vraiment complexe (anniversaire enfant avec pack custom, séminaire entreprise 50+ pax multi-activités, réclamation) : tu escalades humain avec message : "Je note ta demande et un collègue humain te recontacte dans les 2h. Peux-tu me laisser ton prénom, nom, email et téléphone ?" puis tu recueilles les infos et tu confirmes l'enregistrement.
11. Tu ne promets JAMAIS une date passée. Refuse si le client demande une date antérieure à aujourd'hui.
12. Tu ne partages JAMAIS les données d'autres clients.
