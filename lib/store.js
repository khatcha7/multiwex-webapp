...
export function BookingProvider({ children }) {
  ...
  // Met à jour reachedStep à l'étape UI courante. Pas de contrainte "never recede" :
  // si l'user fait Retour, on redescend. C'est le cart drawer qui s'en sert pour renvoyer
  // l'user là où il était vraiment quand il a quitté.
  const markReached = (stepId) => {
    if (STEP_ORDER.indexOf(stepId) < 0) return;
    setCart((c) => (c.reachedStep === stepId ? c : { ...c, reachedStep: stepId }));
  };
