...
// Applique les décalages manuels stockés en localStorage aux créneaux générés
export function applySlotShifts(slots, activityId, dateStr) {
  ...
  try {
    ...
    const actShifts = shifts[key];
    if (!actShifts) return slots;

    const lineShift = actShifts._lineShift || 0;
    const lineFrom = actShifts._lineFrom || '00:00';

    return slots.map((slot) => {
      let shiftMin = 0;
      // Décalage individuel
      if (actShifts[slot.start]) shiftMin += actShifts[slot.start];
      // Décalage de toute l'activité (à partir du créneau spécifié)
      if (lineShift && slot.start >= lineFrom) shiftMin += lineShift;

      if (shiftMin === 0) return slot;

      const startM = toMinutes(slot.start) + shiftMin;
      const endM = toMinutes(slot.end) + shiftMin;
      return { start: fromMinutes(startM), end: fromMinutes(endM) };
    });
  } catch {
    return slots;
  }
