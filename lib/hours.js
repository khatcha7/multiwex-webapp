...
export function parseDate(dateStr) {
  // Midi UTC — toujours dans le même jour calendaire en heure de Bruxelles,
  // évite les effets de bord de fuseau horaire lors du toDateStr roundtrip.
...
export function generateSlotsForActivity(activity, dateStr, { fullDay = false } = {}) {
  ...
  let minStart = openM;
  if (fullDay) {
    // Aligner la stride sur l'heure d'ouverture pour que les slots calendar matchent
    // ceux générés en mode normal (client booking). On commence depuis le 1er multiple
    // de step <= openM (peut être 0 ou plus tôt).
    const baseAnchor = hours ? toMinutes(hours.open) : 0;
    if (baseAnchor > 0) {
      const offset = baseAnchor % step;
      minStart = offset; // 1er slot de la journée aligné modulo step sur openM
    } else {
      minStart = 0;
    }
