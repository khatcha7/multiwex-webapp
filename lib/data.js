...
export async function redeemGiftcard(code, amount, bookingId) {
  const c = (code || '').trim().toUpperCase();
  const card = await getGiftcardByCode(c);
  if (!card) throw new Error('Carte cadeau introuvable');
  if (!card.paid) throw new Error('Carte cadeau non payée');
  if (card.balance < amount) throw new Error('Solde insuffisant');
  const newBalance = card.balance - amount;
  const newRedeemed = [...(card.redeemedBookings || []), bookingId].filter(Boolean);
  if (isSupabaseConfigured) {
    await supabase
      .from('giftcards')
      .update({ balance: newBalance, redeemed_bookings: newRedeemed })
      .eq('code', c);
    return { ...card, balance: newBalance, redeemedBookings: newRedeemed };
  }
  const all = readLS(LS.giftcards, []);
  const idx = all.findIndex((g) => (g.code || '').toUpperCase() === c);
  if (idx >= 0) {
    all[idx] = { ...all[idx], balance: newBalance, redeemedBookings: newRedeemed };
    writeLS(LS.giftcards, all);
    return all[idx];
  }
  return null;
