...
// Pas de 'use client' : ce module est appelé aussi par les routes API server
// (ex: /api/chat/booking pour Ellie). Toutes les fonctions LS sont déjà guardées
// par typeof window === 'undefined'.
...
const defaultConfig = {
  ...
  'chatbot.bot_name': '🤖 Multibot',
  'chatbot.welcome_message': "👋 Bonjour ! Je suis l'assistant Multiwex. Posez-moi vos questions ou cliquez sur une suggestion ci-dessous.",
  'chatbot.starter_suggestions': "Quels sont vos horaires d'ouverture ?\nComment réserver ?\nQuels sont vos tarifs ?",
  'chatbot.bubble_color': '#e8005a',
  'chatbot.position': 'bottom-right', // 'bottom-right' | 'bottom-left'
};
...
export async function createBooking(booking) {
  // A6 — Validation date côté serveur (interdit résa &gt; 1 jour dans le passé sauf source on_site)
  if (booking.date) {
    const today = new Date();
    today.setUTCHours(0, 0, 0, 0);
    const bDate = new Date(booking.date + 'T00:00:00Z');
    const minDate = new Date(today.getTime() - 24 * 3600 * 1000);
    if (bDate < minDate && booking.source !== 'on_site' && booking.source !== 'phone') {
      throw new Error('Date de réservation invalide (dans le passé)');
    }
  }
  // A7 — Normalisation email (lowercase + trim) pour éviter doublons customers
  const normalizedEmail = (booking.customer?.email || '').toLowerCase().trim();
  if (!normalizedEmail) throw new Error('Email client requis');
  ...
  if (isSupabaseConfigured) {
    const { data: customer, error: custErr } = await supabase
      ...
      .single();
    if (custErr || !customer) {
      throw new Error(`Customer upsert failed: ${custErr?.message || 'no data'}`);
    }
...
export async function listBookings({ from, to, customerEmail } = {}) {
  // A7 — Email lowercase pour matcher les customers stockés normalisés
  ...
  if (isSupabaseConfigured) {
    ...
    if (normalizedEmail) q = q.eq('customers.email', normalizedEmail);
    const { data } = await q;
  ...
  if (normalizedEmail) all = all.filter((b) => (b.customer?.email || '').toLowerCase().trim() === normalizedEmail);
...
// A14 — Seed staff par défaut UNIQUEMENT si var d'env explicite est set.
// En prod, l'admin doit créer manuellement son premier staff via SQL Supabase
// pour éviter qu'un compte fictif "admin@multiwex.be" soit accessible publiquement.
const SHOULD_SEED_DEMO_STAFF = process.env.NEXT_PUBLIC_SEED_DEMO_DATA === 'true';
...
export async function initStaffUsers() {
  ...
  if (isSupabaseConfigured) {
    ...
    if (data && data.length > 0) {
    ...
    } else if (SHOULD_SEED_DEMO_STAFF) {
      // Seed defaults uniquement en dev (NEXT_PUBLIC_SEED_DEMO_DATA=true)
      ...
      _staffCache = []; // Aucun staff seedé en prod sans var d'env
    }
  } else {
    const stored = readLS(LS.staffUsers, null);
    if (!stored && SHOULD_SEED_DEMO_STAFF) writeLS(LS.staffUsers, DEFAULT_STAFF);
    _staffCache = stored || (SHOULD_SEED_DEMO_STAFF ? DEFAULT_STAFF : []);
  }
  return _staffCache;
}
...
export function listStaffUsers() {
  ...
  const stored = readLS(LS.staffUsers, null);
  return stored || (SHOULD_SEED_DEMO_STAFF ? DEFAULT_STAFF : []);
}
...
// A12 — Helper booléen tolérant : true / 'true' / 1 / '1' / 'on' / 'yes' tous truthy
// Évite les bugs où Supabase JSONB renvoie soit true natif soit 'true' string selon contexte.
export function getBool(key, defaultVal = false) {
...
function normalizeGiftcard(g) {
  return {
    ...
    message: g.message,
    paid: g.paid,
    redeemedBookings: g.redeemed_bookings || [],
    createdAt: g.created_at,
    expiresAt: g.expires_at,
  };
}

export async function listGiftcards({ email } = {}) {
  if (isSupabaseConfigured) {
    let q = supabase.from('giftcards').select('*').order('created_at', { ascending: false });
    if (email) q = q.eq('to_email', email);
    const { data } = await q;
    return (data || []).map(normalizeGiftcard);
  }
  let arr = readLS(LS.giftcards, []);
  if (email) arr = arr.filter((g) => (g.toEmail || '').toLowerCase() === email.toLowerCase());
  return arr;
}
...
export async function getGiftcardByCode(code) {
  ...
  if (!c) return null;
  if (isSupabaseConfigured) {
    const { data } = await supabase.from('giftcards').select('*').eq('code', c).maybeSingle();
    return data ? normalizeGiftcard(data) : null;
  }
  const all = readLS(LS.giftcards, []);
  return all.find((g) => (g.code || '').toUpperCase() === c) || null;
}
...
export async function createGiftcard(card) {
  const payload = {
    ...
    from_name: card.fromName || null,
    to_name: card.toName || null,
    to_email: card.toEmail || null,
    message: card.message || null,
    paid: !!card.paid,
    expires_at: card.expiresAt || null,
  };
  if (isSupabaseConfigured) {
    const { data, error } = await supabase.from('giftcards').insert(payload).select().single();
    if (error) throw error;
    return normalizeGiftcard(data);
  }
  const all = readLS(LS.giftcards, []);
  const created = { id: crypto.randomUUID(), ...card, balance: card.balance ?? card.amount, createdAt: new Date().toISOString() };
  all.push(created);
  writeLS(LS.giftcards, all);
  return created;
}

export async function redeemGiftcard(code, amount, bookingId) {
  const c = (code || '').trim().toUpperCase();
  if (!c) throw new Error('Code requis');
  if (!(amount > 0)) throw new Error('Montant invalide');

  if (isSupabaseConfigured) {
    // A8 — Update atomique : la condition .gte('balance', amount) garantit
    // qu'aucune redemption concurrente ne peut faire passer le solde en négatif.
    // Si une autre requête a déjà décrémenté → balance < amount → 0 ligne updated.
    const { data: existing, error: readErr } = await supabase
      .from('giftcards')
      .select('balance, paid, redeemed_bookings')
      .eq('code', c)
      .maybeSingle();
    if (readErr || !existing) throw new Error('Carte cadeau introuvable');
    if (!existing.paid) throw new Error('Carte cadeau non payée');

    const newBalance = parseFloat(existing.balance) - amount;
    const newRedeemed = [...(existing.redeemed_bookings || []), bookingId].filter(Boolean);

    const { data: updated, error: updErr } = await supabase
      .from('giftcards')
      .update({ balance: newBalance, redeemed_bookings: newRedeemed })
      .eq('code', c)
      .gte('balance', amount) // ← garde-fou atomique : refuse si balance courante < amount
      .select()
      .maybeSingle();

    if (updErr) throw updErr;
    if (!updated) throw new Error('Solde insuffisant ou conflit concurrent');
    return normalizeGiftcard(updated);
  }

  // Fallback LS — pas de concurrence multi-tab vraie sur LS, lecture/écriture séquentielle suffit
  const all = readLS(LS.giftcards, []);
  const idx = all.findIndex((g) => (g.code || '').toUpperCase() === c);
  if (idx < 0) throw new Error('Carte cadeau introuvable');
  if (!all[idx].paid) throw new Error('Carte cadeau non payée');
  if ((all[idx].balance ?? all[idx].amount) < amount) throw new Error('Solde insuffisant');
  all[idx] = {
    ...all[idx],
    balance: (all[idx].balance ?? all[idx].amount) - amount,
    redeemedBookings: [...(all[idx].redeemedBookings || []), bookingId].filter(Boolean),
  };
  writeLS(LS.giftcards, all);
  return all[idx];
}
...
export async function migrateLocalStorageToSupabase() {
  ...
  const report = { config: 0, popups: 0, staff: 0, slot_blocks: 0, notes: 0, giftcards: 0, errors: [] };
  ...
  // 6. giftcards
  try {
    const cards = readLS(LS.giftcards, []);
    for (const c of cards) {
      const { data: existing } = await supabase.from('giftcards').select('id').eq('code', c.code).maybeSingle();
      if (!existing) {
        await supabase.from('giftcards').insert({
          code: c.code,
          amount: c.amount,
          balance: c.balance ?? c.amount,
          from_name: c.fromName || null,
          to_name: c.toName || null,
  ...
  } catch (e) { report.errors.push(`giftcards: ${e.message}`); }
