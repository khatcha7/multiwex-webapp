...
export async function getSlotOccupancy(activityId, dateStr) {
  ...
  const fakeOccupied = getFakeOccupiedSlots(activity, dateStr);
  fakeOccupied.forEach((start) => {
    if (!map[start]) map[start] = { groups: 1, players: Math.ceil(activity.maxPlayers * (0.5 + Math.random() * 0.5)) };
  });
  return map;
}
