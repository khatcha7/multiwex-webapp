...
// Met à jour une note / label sur un bloc (ou un ensemble de blocs par batchId)
export async function updateSlotBlock(id, updates) {
  if (isSupabaseConfigured) {
    const { data } = await supabase.from('slots').update(updates).eq('id', id).select().single();
    return data;
  }
  const all = readLS(LS.slotBlocks, []);
...
export async function unblockBatch(batchId) {
  const all = readLS(LS.slotBlocks, []);
  writeLS(LS.slotBlocks, all.filter((b) => b.batchId !== batchId));
}
...
export async function updateNote(id, updates) {
  ...
  const payload = {
    ...
    ...(updates.locked !== undefined && { locked: updates.locked, locked_by_staff_id: updates.locked ? (staff?.id || null) : null }),
    updated_by_staff_id: staff?.id || null,
    updated_by_name: staff?.name || null,
